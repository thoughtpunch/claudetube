    ...
    **claudetube supports 1,500+ video sites** through yt-dlp, not just YouTube. This includes:
    ...
    YouTube enforces PO (Proof of Origin) tokens and SABR streaming, which can cause 403 errors or missing formats. claudetube currently works for most public videos via the `android_vr` client (no tokens needed), but this may change as YouTube continues tightening access.

    If a YouTube download fails with 403:
    1. Ensure `deno` is installed (`brew install deno`) -- required by yt-dlp for JS challenge solving
    2. Check if cookies or PO tokens are configured in `.claudetube/config.yaml`
    3. Direct the user to `documentation/guides/youtube-auth.md` for setup instructions

    YouTube auth config lives under the `youtube:` key in config:
    ```yaml
    youtube:
      cookies_file: "~/.config/claudetube/youtube-cookies.txt"
      po_token: "mweb.gvs+TOKEN"           # Manual, expires ~12hr
